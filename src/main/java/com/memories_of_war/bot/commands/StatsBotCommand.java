...
public class StatsBotCommand implements IBotCommand {
	...
	public String execute(String[] tokenizedMessage, MessageReceivedEvent event) {
		...
		// return error message if the user has no DiscordUser registered.
		if(users.size() == 0)
			return mention + "Error: User is not registered. Use the !join <username> command first.";
		
		// since discordId has a uniqueness guarantee from the schema, there should be only 1 or 0 results.
