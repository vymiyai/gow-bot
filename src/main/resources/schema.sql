...
create table if not exists public.discord_resources (id  bigserial not null, cooldown timestamp with time zone, gems int4, gold int4, spent_gems int4, spent_gold int4, v2launched int4, primary key (id));
create table if not exists public.discord_user (id  bigserial not null, access_token varchar(255), creation_date timestamp with time zone, discord_id int8, discord_username varchar(255), last_change timestamp with time zone, discord_resources_fk int8, primary key (id));
...
alter table discord_user ADD COLUMN IF NOT EXISTS description varchar(511);